...
team_chances = []
for teamname in final_sorted_teamnames:
    team_chances.append([teamname,round(probability_upto_n(teamname,2),2),round(probability_upto_n(teamname,4),2)])
    #print(teamname+'\t\t\t'+str(round(top_x_chance(teamname,2),2))+'\t\t\t'+str(round(top_x_chance(teamname,4),2)))

topchances_df = pd.DataFrame(team_chances,columns=['team','top 2','top 4'])
